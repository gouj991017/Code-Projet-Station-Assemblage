...
public class UI_Base extends javax.swing.JFrame
{
    ...
    enum enumBacs { //Attribue des bacs pour chaque composants d'un produit
        baseType1Couleur1,  //0
        crayonType1,
        baseType1Couleur2,
        baseType2Couleur1,
    ...
    public static void main(String args[])
    {
        ...
        try
        {
            ...
            try
            {
                ...
                while(!brFluxDonnees.readLine().trim().equals("ITEM_WEIGHT ="));  //Attente de ITEM_WEIGHT.
                param_poidPiece = new Integer(brFluxDonnees.readLine()).intValue();    //Lecture du paramètre de trigger.
        ...
        thread_Recep_MQTT.start();  //lance la réception de commandes.
        
        logiqueEtape(commande, true);   //Charge l'étape 0.
    ...
    public static int logiqueEtape(ObjCommande cmd, boolean publish)
    {
        ...
        switch (etape_courante)
        {
            case 0:
                ...
                break;
            case 1:
                if(cmd.typeBase_ == 1) //Base type 1
                {
                    if(cmd.couleurBase_.equals("blanc"))
                    {
                        bacCourant = enumBacs.baseType1Couleur1.ordinal();
                    }
                    else if(cmd.couleurBase_.equals("noir"))
                    {
                        bacCourant = enumBacs.baseType1Couleur2.ordinal();
                    }
                }
                else if(cmd.typeBase_ == 2) //Base type 2
                {
                    if(cmd.couleurBase_.equals("blanc"))
                    {
                        bacCourant = enumBacs.baseType2Couleur1.ordinal();
                    }
                    else if(cmd.couleurBase_.equals("noir"))
                    {
                        bacCourant = enumBacs.baseType2Couleur2.ordinal();
                    }
                }
                surligneBac(bacCourant);
                messageBaseJsonObj.put("Message", baseProd(cmd.typeBase_,cmd.couleurBase_,bacCourant));
                break;
            case 2:
                switch (cmd.typeCrayon_)
                {
                //Porte-crayon type 1
                    case 1:
                        bacCourant = enumBacs.crayonType1.ordinal();
                        break;
                //Porte-crayon type 2
                    case 2:
                        bacCourant = enumBacs.crayonType2.ordinal();
                        break;
                //Porte-crayon type 3
                    case 3:
                        bacCourant = enumBacs.crayonType3.ordinal();
                        break;
                }
                surligneBac(bacCourant);    //Affiche le bac dans lequel il faudra prendre les pièces.
                messageBaseJsonObj.put("Message", crayonProd(cmd.qteCrayon_,cmd.typeCrayon_,bacCourant));
                break;
            case 3:
                switch (cmd.typeSupport_)
                {
                //Supports courts
                    case 1:
                        bacCourant = enumBacs.supportCourt.ordinal();
                        break;
                //Supports moyen
                    case 2:
                        bacCourant = enumBacs.supportMoyen.ordinal();
                        break;
                //Aucun support
                    default:
                ...
                {
                    surligneBac(bacCourant);    //Affiche le bac dans lequel il faudra prendre les pièces.
                    messageBaseJsonObj.put("Message", supportsProd(cmd.typeSupport_,bacCourant));
                }
                break;
        }
    ...
    */
    ...
    {
        String statement = "Prenez la Base de type " + iBase + " de couleur " + iCouleur + " se trouvant dans le bac #" + (iBacCourant+1);
        return statement;
    }
    ...
    */
    ...
    {
        String statement = "Prenez " + iqteCrayon +" porte-crayon de type "+ iCrayon + " dans le bac #" + (iBacCourant+1);
        return statement;
    }
    ...
    */
    ...
    {
        String statement = "Prenez deux support de type " + iSupports + "dans le bac #" + (iBacCourant+1);
        return statement;
    }
            ...
            while(true)
            {
                try
                {
                    ...
                    if (message.getPayload() != null)
                    {
                        ...
                        tbBase.setText(Integer.toString(commande.typeBase_));
                        tbCouleur.setText(couleur);
                        tbCrayon.setText(Integer.toString(commande.typeCrayon_));
